...
Consider the sequence of parameters $\{\phi_k\}$ generated by the gradient ascent update rule $\phi_{k+1} = \phi_k + \lambda \nabla J(\phi_k)$ (Eq.~\ref{eqn:phi_update} applied in inner loop of Alg.~\ref{Main Alg}). If the step size $\lambda$ satisfies $0 < \lambda < \frac{2}{L}$ ($L$ defined in Eq.~\ref{eqn:L}), then the sequence converges to a stationary point, i.e.,
\[ \lim_{k \to \infty} \|\nabla J(\phi_k)\| = 0 \]
